...
// Modified by Bernard Klinc (Nicer output)
...
int main (int argc, char **argv)
{
  ...
  char ioctl_location[256] = "Unknown";
  int  ioctl_location_ret;
  char ioctl_identifier[256] = "Unknown";
  int  ioctl_identifier_ret;
  char ioctl_properties[256] = "Unknown";
  int  ioctl_properties_ret;
  int abs[5];
  ...
  errno = 0;
  ioctl_location_ret = ioctl(fd, EVIOCGPHYS(sizeof(ioctl_location)), ioctl_location);
  if(errno)
    sprintf(ioctl_location, "ERR %s", strerror(errno));
  ...
  while (1) {
    ...
    for (i = 0; i < rd / sizeof(struct input_event); i++) {
        if(ev[i].type == EV_SYN && ev[i].code == EV_SYN)
        {
          printf("<<<Sync>>\n");
          continue;
        }

        printf("T%6d Type: %d (%-8s)   Code: %d (%-4s)   Value: 0x%02x(%d)\n",
